...
impl App {
    ...
    fn focus_pane_after_input(&mut self, pane: PaneId) {
        if self.prepare_pty_input_before_mutation() {
            ...
            // Remote frontends update their mirror optimistically above, but
            // focus is still shared session state. Forward the mutation so
            // native frontends receive the resulting tree-changed event.
